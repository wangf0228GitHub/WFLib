...
namespace WFNetLib.USB.HID
{
    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
			...
			if ( !m_hHandle.IsInvalid)	// if the open worked...
			{
				...
				if (HidD_GetPreparsedData(m_hHandle.DangerousGetHandle(), out lpData))	// get windows to read the device data into an internal buffer
				{
					try
					{
                        ...
                        HIDStream = new FileStream(m_hHandle, FileAccess.Read | FileAccess.Write, m_nInputReportLength, true);	// wrap the file handle in a .Net file stream                         
        ...
        public void BeginAsyncRead()
        {
            ...
            ReadIAsync=HIDStream.BeginRead(arrInputReport, 0, m_nInputReportLength, new AsyncCallback(ReadCompleted), arrInputReport);
        }
		/// <summary>
        ...
        protected void ReadCompleted(IAsyncResult iResult)
        {
            ...
            try
            {
                HIDStream.EndRead(iResult);	// call end read : this throws any exceptions that happened during the read
                try
                {
        ...
        protected void Write(OutputReport oOutRep)
        {
			try
			{
				HIDStream.Write(oOutRep.Buffer, 0, oOutRep.BufferLength);
			}
...
// 		}
		...
		#endregion
        string hidPath;
        public byte[] DeviceInteraction(byte[] tx, int stepTime,int n)
        {
            try
            {                
                HIDStream.Write(tx, 0, m_nOutputReportLength);
                ...
                for (int s = 0; s < n; s++)
                {
                    if (ia.AsyncWaitHandle.WaitOne(stepTime, false))
                    {
                        return InputReport;
                    }
                }
        ...
        public byte[] DeviceInteraction(byte[] tx)
        {
            return DeviceInteraction(tx, 3000,1);
        }
        public byte[] DeviceReceive(int stepTime, int n)
        {
            try
            {
                byte[] InputReport = new byte[m_nInputReportLength];
        ...
        public byte[] DeviceReceive(byte[] tx)
        {
            return DeviceReceive(3000, 1);
        }
    }
}
