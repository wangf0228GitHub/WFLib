...
{
    public class DataGridView_Summary : UserControl
    {
        #region 公有属性

        private bool _bDisplaySumRowHeader;
        /// <summary>
        /// 是否显示合计行标题
        /// </summary>
        public bool DisplaySumRowHeader
        {
            get { return _bDisplaySumRowHeader; }
            set { _bDisplaySumRowHeader = value; }
        }
        ...
        private bool _bSumRowHeaderTextBold;
        /// <summary>
        /// 合计列标题用粗体显示
        /// </summary>
        public bool SumRowHeaderTextBold
        {
            get { return _bSumRowHeaderTextBold; }
            set { _bSumRowHeaderTextBold = value; }
        }
        ...
        #region 构造函数
        public DataGridView_Summary(DataGridView dgv, string[] summaryColumns)
            : this(dgv, true, "合计", false, summaryColumns)
        {
        }
        public DataGridView_Summary(DataGridView dgv, bool bDisplaySumRowHeader, string sumRowHeaderText,bool bSumRowHeaderTextBold, string[] summaryColumns)
        {
            if (dgv == null)
            {
            ...
            _bDisplaySumRowHeader = bDisplaySumRowHeader;
            _SumRowHeaderText = sumRowHeaderText;
            _bSumRowHeaderTextBold = bSumRowHeaderTextBold;
                    ...
                    foreach (DataGridViewCell dgvCell in dgvRow.Cells)
                    {
                        foreach (DataGridViewColumn dgvColumn in sumBoxHash.Keys)
                        {
                            if (dgvCell.OwningColumn.Equals(dgvColumn))
                            {
                                ...
                                if (sumBox != null && sumBox.bSummary)
                                {
        ...
        private void reCreateSumBoxes()
        {
            ...
            foreach (DataGridViewColumn dgvColumn in sortedColumns)
            {
                ...
                if (SummaryColumns != null && SummaryColumns.Length > 0)
                {
                    ...
                    {
                        if (SummaryColumns[iCntX] == dgvColumn.DataPropertyName || SummaryColumns[iCntX] == dgvColumn.Name)
                        {
                            sumBox.TextAlign = TextHelper.TranslateGridColumnAligment(dgvColumn.DefaultCellStyle.Alignment);
                            sumBox.bSummary = true;
