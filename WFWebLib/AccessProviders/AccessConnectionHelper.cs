...
namespace WFWebLib.AccessProviders
{
    internal static class AccessConnectionHelper
    {
        ...
        {
            OleDbCommand lookupCommand = new OleDbCommand("SELECT PathId FROM aspnet_Paths WHERE ApplicationId = @AppId AND Path = @Path", connection);
            ...
            if (createIfNeeded)
            {
                OleDbCommand createCommand = new OleDbCommand("INSERT INTO aspnet_Paths (ApplicationId, Path) VALUES (@AppID, @Path)", connection);
        ...
        internal static int GetUserID(OleDbConnection connection, int applicationID, string userName, bool createIfNeeded, bool newUserIsAnonymous, DateTime lastActivityDate)
        {
            ...
            OleDbCommand lookupCommand = new OleDbCommand(@"SELECT UserId FROM aspnet_Users WHERE ApplicationId = @AppId AND UserName = @UserName", connection);
            lookupCommand.Parameters.Add(new OleDbParameter("@AppId", applicationID));
