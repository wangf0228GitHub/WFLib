...
namespace WFWebLib.AccessProviders
{
    public class AccessMembershipProvider : MembershipProvider
    {
        ...
        public override bool ChangePasswordQuestionAndAnswer(string username, string password, string newPasswordQuestion, string newPasswordAnswer)
        {
            ...
            try
            {
                try
                {
                    ...
                    ////////////////////////////////////////////////////////////
                    // Step 3: Update table
                    command = new OleDbCommand(@"UPDATE aspnet_Membership " + @"SET PasswordQuestion = @PasswordQuestion, PasswordAnswer = @PasswordAnswer " + @"WHERE UserId = @UserId", connection);
        ...
        public override int GetNumberOfUsersOnline()
        {
            ...
            try
            {
                try
                {
                    ...
                    command = new OleDbCommand(@"SELECT COUNT(*) FROM aspnet_Users u, aspnet_Membership m WHERE u.UserId = m.UserId AND ApplicationId=@AppId AND LastActivityDate > @LastActivityDate", connection);
                    command.Parameters.Add(new OleDbParameter("@AppId", appId));
                    command.Parameters.Add(CreateDateTimeOleDbParameter("@LastActivityDate", dt));
                    result = command.ExecuteScalar();
                            ...
                            out bool userIsApproved)
        {
            ...
            command = new OleDbCommand(@"SELECT PasswordFormat, [Password], PasswordAnswer, PasswordSalt, IsApproved " +
                                        @"FROM aspnet_Membership m, aspnet_Users u " +
                                        @"WHERE m.UserId = @UserId AND m.UserId = u.UserId", connection);
            command.Parameters.Add(new OleDbParameter("@UserId", userId));
